...
from worldsmith.simulation import World, create_demo_world
...
class FeasibilityTests(unittest.TestCase):
    ...
    def test_corrupt_and_version_mismatch_saves_fail_clearly(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "bad.json"; path.write_text("not-json")
            with self.assertRaises(SaveError): load_world(path)
    ...
    def test_intervention_has_understandable_downstream_difference(self) -> None:
        ...
        changed.intervene("miracle", sorted(changed.settlements)[0], 30); changed.advance(200)
    ...
    def test_belief_can_diverge_from_truth(self) -> None:
        world = create_demo_world(); event = world.intervene("natural_disaster", sorted(world.settlements)[0], 8)
    ...
    def test_branch_preserves_prime_and_progresses_independently(self) -> None:
        prime = create_demo_world(); prime.advance(200); point = prime.events[-1].id
        branch = prime.branch(point, "branch-a")
        before = prime.to_dict(); branch.intervene("bless", sorted(branch.civilizations)[0], 10); branch.advance(40)
    ...
    def test_chronicle_is_derived_and_filterable(self) -> None:
        world = create_demo_world(); world.advance(30)
        self.assertTrue(world.chronicle())
        self.assertTrue(all("migration" in row for row in world.chronicle(event_type="migration")))

    def test_event_ledger_has_stable_fields_and_required_event_types(self) -> None:
        world = create_demo_world(); world.advance(100)
        world.year = 22
        next(iter(world.relationships.values())).score = -99
        world.advance(1)
        collapsed = next(iter(world.civilizations.values())); collapsed.population = 0
        world.advance(1)
        world.intervene("natural_disaster", sorted(world.settlements)[0], 1)
        for event_type in ("migration", "famine", "religious_movement"):
            world.emit(event_type, [collapsed.id], None, [], {}, 2, f"Ledger test: {event_type}.")
        kinds = {event.type for event in world.events}
        self.assertTrue({"settlement_founded", "ruler_death", "war_declared", "war_concluded",
                         "migration", "famine", "religious_movement", "civilization_collapse",
                         "natural_disaster"}.issubset(kinds))
    ...
    def test_500_year_run_is_preliminarily_performant(self) -> None:
        start = time.perf_counter(); world = create_demo_world(); world.advance(500)
