...
class World:
    ...
    def _advance_year(self) -> None:
        ...
        for civilization_id in sorted(self.civilizations):
            ...
            if civ.population <= 0:
                if not any(
                    event.type == "civilization_collapse"
                    and civ.id in event.participants
            ...
            if settlements:
                ...
                target.resources = max(
                    0, target.resources + rng.randint(-1, 3) - pressure
                )
            ...
            if self.year % 97 == 0:
                ...
                successor = Person(
                    self.entity_id("person"), f"{civ.name} Successor", civ.id
                )
    ...
    def _migration(self, rng: Random) -> None:
        ...
        for civ in sorted(self.civilizations.values(), key=lambda item: item.id):
            if len(civ.settlement_ids) < 2 or civ.population < 20:
                continue
            source, target = (
                self.settlements[s] for s in sorted(civ.settlement_ids)[:2]
            )
            ...
            self.emit(
                ...
                {"moved": moved},
                2,
                f"People migrated within {civ.name}.",
            )
...
def create_demo_world(seed: int = 42) -> World:
    ...
    for index, name in enumerate(("Velar", "Ordan", "Tes"), 1):
        ...
        religion = Religion(
            world.entity_id("religion"), f"{name} Tradition", civ_id, "ancestral memory"
        )
        ...
        civ = Civilization(
            civ_id, name, settlement.population, person.id, [settlement.id], religion.id
        )
        ...
        resource = Resource(
            world.entity_id("resource"), "grain", settlement.region_id, 100
        )
